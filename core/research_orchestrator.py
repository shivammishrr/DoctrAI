...
class MedicalResearchOrchestrator:
    ...
    def _generate_research_questions(self, initial_query: str) -> List[str]:
        ...
        questions = [match.group(1).strip() for line in content.split('\n') if (match := re.match(r"^\d+\.\s*(.*)", line.strip()))]
    ...
    def _research_single_question_with_tools(self, question: str) -> str:
        ...
        current_turn_messages = [{"role": "system", "content": SYSTEM_MESSAGE_TOOL_USER}] + [
                ...
                for tool_call in response_message.tool_calls:
                    ...
                    try:
                        ...
                        if function_name in self.tool_executor.dispatch_table:
                            query_arg = next((args_dict.get(key) for key in ["query", "search_query", "topic"] if key in args_dict), None)
                            if query_arg is None: raise ValueError("Missing required 'query' argument.")
                    ...
                    current_turn_messages.append({"role": "tool", "tool_call_id": tool_call_id, "name": function_name, "content": tool_result})
        ...
        response_message, model_used = self.model_manager.create_completion(messages=synthesis_messages, reasoning_level="high", temperature=0.5, max_tokens=1500)
        return response_message.content if response_message and response_message.content else "\n\n".join(accumulated_tool_results)
    ...
    def _critique_findings(self, findings_text: str, original_query: str) -> str:
        ...
        self._report_progress(f"LLM Call: Critiquing findings for: '{original_query}'")
        self._update_research_state({"status_message": "Critiquing research findings..."})
        
        prompt = CRITIQUE_RESEARCH_FINDINGS_PROMPT.format(original_query=original_query, findings_text=findings_text)
        response_message, _ = self.model_manager.create_completion(messages=[{"role": "system", "content": SYSTEM_MESSAGE_RESEARCH_VALIDATOR}, {"role": "user", "content": prompt}], reasoning_level="high", temperature=0.3, max_tokens=800)
    ...
    def _generate_final_report(self, initial_query: str, research_summary_per_question: Dict[str, str], overall_critique: str) -> str:
        ...
        formatted_research = "\n\n".join(f"Question: {q}\nResearch Summary:\n{res}" for q, res in research_summary_per_question.items())
        prompt = GENERATE_FINAL_REPORT_PROMPT.format(initial_query=initial_query, formatted_research=formatted_research, overall_critique=overall_critique)
        response_message, _ = self.model_manager.create_completion(messages=[{"role": "system", "content": SYSTEM_MESSAGE_REPORT_WRITER}, {"role": "user", "content": prompt}], reasoning_level="high", temperature=0.4, max_tokens=3000)

        if response_message and response_message.content:
            report_content = re.sub(r'<[^>]+>', '', response_message.content)
    ...
    def run_research_workflow(self, initial_query: str, result_container: Dict[str, Any]) -> None:
        """
        Main method to run the workflow. Designed to be run in a thread.
        It places the final result into the shared `result_container`.
        """
        try:
            self.current_research_state = {"initial_query": initial_query}
            ...
            collated_syntheses = "\n\n---\n\n".join(f"Question: {q}\nSynthesized Answer: {ans}" for q, ans in all_question_research_syntheses.items())
            ...
            result_container['report'] = f"I apologize, but I encountered an unexpected error during the deep research process: {str(e)}"